function submitthor(fun, inputs, nOut, additionalFun, name, time, proc, mem, jvm)
%function submit(fun, inputs, nOut, additionalFun, name, time, proc)
% Uses general submit script submit.maui.general
%SUBMITTHOR Submit matlab jobs to the Tidor Thor cluster
%
%   submitthor(fun, inputs, nOut, additionalFun, name, time, proc)
%
%   Inputs:
...
%       Essentially, submitthor does this for all i:
%       
...
if isempty(time)
    error('submitthor:ExpectedTime', 'You must specify a maximum time.');
end
...
% Number processors
if proc > 12
    error('submitthor:ProcessorNumber', 'Too many processors desired.')
end
...
for i = 1:N
    ...
    while strcmp(status(1), 'N') || strcmp(status(1), 'E')
        [result status] = system(['ssh thor "cd ' dir ' && ' script ' -r "' dir 'run_submitfile.sh ' matlabDir ' ' num2str(i) '""']);
        fprintf(status)
    end
    jobNames{i} = regexp(status, '\d{1,}\.thor', 'match', 'once');
end
...
fprintf(fid, '    system("ssh thor qdel $line");\n');
fprintf(fid, '}\n');
fclose(fid);
...
fid = fopen([dir 'resubmit.thor.here'], 'w');
...
fprintf(fid, 'queueout=`qsum-thor.pl -J`\n');
fprintf(fid, '\n');
...
fprintf(fid, '    then\n');
fprintf(fid, ['        commandstring="ssh thor cd ${PWD} && ' script ' ${PWD}/run_submitfile.sh ' matlabDir ' $i"\n']);
...
system(['chmod 744 "' dir 'resubmit.thor.here"']);
